 ...
 * @file
 */
#ifndef MCA_BTL_TIPC_ADDR_H
#define MCA_BTL_TIPC_ADDR_H
 ...
 * the local BTL TIPC component. This is used to construct an opal_if_t
 * structure for the reachability component as well as populate the
 * mca_btl_tipc_addr_t structure on remote procs. These will be used
 * for interface matching and filling out the mca_btl_base_endpoint_t
 * structure.
 */
struct mca_btl_tipc_modex_addr_t {
    uint8_t addr[16];        /* endpoint address.  for addr_family
                                of MCA_BTL_TIPC_AF_INET, only the
    ...
    uint8_t addr_family;     /* endpoint address family.  Note that
                                this is
                                MCA_BTL_TIPC_AF_{INET,INET6}, not
                                the traditional
                                AF_INET/AF_INET6. */
    uint8_t padding[1];      /* pad out to an 8-byte word */
};
typedef struct mca_btl_tipc_modex_addr_t mca_btl_tipc_modex_addr_t;

#if (__STDC_VERSION__ >= 201112L)
_Static_assert(sizeof(struct mca_btl_tipc_modex_addr_t) == 32, "mca_btl_tipc_modex_addr_t");
#endif
 ...
 * mca_btl_tipc_modex_addr_t structure.
 */
struct mca_btl_tipc_addr_t {
    union {
...
// #endif
#ifdef FIDEL
        struct tipc_name addr_tipc; /* TIPC address */
#endif
    } addr_union;
...
typedef struct mca_btl_tipc_addr_t mca_btl_tipc_addr_t;

#define MCA_BTL_TIPC_AF_INET  0
#define MCA_BTL_TIPC_AF_INET6 1
#ifdef FIDEL
#define MCA_BTL_TIPC_AF_TIPC  2
#endif
