...
static int mca_btl_tipc_register_error_cb(struct mca_btl_base_module_t *btl,
                                         mca_btl_base_module_error_cb_fn_t cbfunc);

mca_btl_tipc_module_t mca_btl_tipc_module =
    {.super =
         {
             .btl_component = &mca_btl_tipc_component.super,
             .btl_add_procs = mca_btl_tipc_add_procs,
             .btl_del_procs = mca_btl_tipc_del_procs,
             .btl_finalize = mca_btl_tipc_finalize,
             .btl_alloc = mca_btl_tipc_alloc,
             .btl_free = mca_btl_tipc_free,
             .btl_prepare_src = mca_btl_tipc_prepare_src,
             .btl_send = mca_btl_tipc_send,
             .btl_put = mca_btl_tipc_put,
             .btl_dump = mca_btl_base_dump,
             .btl_register_error = mca_btl_tipc_register_error_cb, /* register error */
         },
     .tipc_endpoints_mutex = OPAL_MUTEX_STATIC_INIT};

static int mca_btl_tipc_register_error_cb(struct mca_btl_base_module_t *btl,
                                         mca_btl_base_module_error_cb_fn_t cbfunc)
{
    mca_btl_tipc_module_t *tipc_btl = (mca_btl_tipc_module_t *) btl;
    tipc_btl->tipc_error_cb = cbfunc;
    return OPAL_SUCCESS;
}
...
int mca_btl_tipc_add_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                          ...
                          opal_bitmap_t *reachable)
{
    ...
    for (i = 0; i < (int) nprocs; i++) {
        ...
        if (NULL == (tipc_proc = mca_btl_tipc_proc_create(opal_proc))) {
            continue;
        }

        OPAL_THREAD_LOCK(&tipc_proc->proc_lock);

        for (uint32_t j = 0; j < (uint32_t) tipc_proc->proc_endpoint_count; ++j) {
            tipc_endpoint = tipc_proc->proc_endpoints[j];
            if (tipc_endpoint->endpoint_btl == tipc_btl) {
                existing_found = true;
                break;
            }
        }
        ...
        if (!existing_found) {
            ...
            OPAL_THREAD_LOCK(&tipc_btl->tipc_endpoints_mutex);
            opal_list_append(&tipc_btl->tipc_endpoints, (opal_list_item_t *) tipc_endpoint);
            OPAL_THREAD_UNLOCK(&tipc_btl->tipc_endpoints_mutex);
        }
...
int mca_btl_tipc_del_procs(struct mca_btl_base_module_t *btl, size_t nprocs,
                          struct opal_proc_t **procs, struct mca_btl_base_endpoint_t **endpoints)
{
    ...
    OPAL_THREAD_LOCK(&tipc_btl->tipc_endpoints_mutex);
    for (i = 0; i < nprocs; i++) {
        mca_btl_tipc_endpoint_t *tipc_endpoint = endpoints[i];
        opal_list_remove_item(&tipc_btl->tipc_endpoints, (opal_list_item_t *) tipc_endpoint);
        OBJ_RELEASE(tipc_endpoint);
    }
    OPAL_THREAD_UNLOCK(&tipc_btl->tipc_endpoints_mutex);
    return OPAL_SUCCESS;
...
mca_btl_base_descriptor_t *mca_btl_tipc_alloc(struct mca_btl_base_module_t *btl,
...
int mca_btl_tipc_free(struct mca_btl_base_module_t *btl, mca_btl_base_descriptor_t *des)
{
...
int mca_btl_tipc_send(struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                     struct mca_btl_base_descriptor_t *descriptor, mca_btl_base_tag_t tag)
{
    ...
    if (endpoint->endpoint_nbo) {
        MCA_BTL_TIPC_HDR_HTON(frag->hdr);
    }
...
{
    mca_btl_tipc_frag_t *frag = (mca_btl_tipc_frag_t *) desc;
                    ...
                    void *cbdata)
{
    ...
    MCA_BTL_TIPC_FRAG_ALLOC_USER(frag);
    if (OPAL_UNLIKELY(NULL == frag)) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    frag->hdr.type = MCA_BTL_TIPC_HDR_TYPE_PUT;
    frag->hdr.count = 1;
    if (endpoint->endpoint_nbo) {
        MCA_BTL_TIPC_HDR_HTON(frag->hdr);
    }
    return ((i = mca_btl_tipc_endpoint_send(endpoint, frag)) >= 0 ? OPAL_SUCCESS : i);
}
                    ...
                    void *cbdata)
{
     ...
     * tipc so the extra latency is not an issue */
    frag->base.des_flags = MCA_BTL_DES_FLAGS_BTL_OWNERSHIP | MCA_BTL_DES_SEND_ALWAYS_CALLBACK;
    frag->base.des_cbfunc = fake_rdma_complete;
    ...
    frag->iov[1].iov_len = sizeof(mca_btl_base_segment_t);
    frag->hdr.base.tag = MCA_BTL_TAG_BTL;
    frag->hdr.type = MCA_BTL_TIPC_HDR_TYPE_GET;
    frag->hdr.count = 1;
    if (endpoint->endpoint_nbo) {
        MCA_BTL_TIPC_HDR_HTON(frag->hdr);
    }
    return ((rc = mca_btl_tipc_endpoint_send(endpoint, frag)) >= 0 ? OPAL_SUCCESS : rc);
}
...
int mca_btl_tipc_finalize(struct mca_btl_base_module_t *btl)
{
    ...
    /* Don't lock the tipc_endpoints_mutex, at this point a single
     * thread should be active.
     */
    for (item = opal_list_remove_first(&tipc_btl->tipc_endpoints); item != NULL;
...
void mca_btl_tipc_dump(struct mca_btl_base_module_t *base_btl,
                      struct mca_btl_base_endpoint_t *endpoint, int verbose)
{
    mca_btl_tipc_module_t *btl = (mca_btl_tipc_module_t *) base_btl;
    mca_btl_base_err("%s TIPC %p kernel_id %d\n"
#if MCA_BTL_TIPC_STATISTICS
                     " |   statistics: sent %lu recv %lu\n"
#endif /* MCA_BTL_TIPC_STATISTICS */
                     " |   latency %u bandwidth %u\n",
                     OPAL_NAME_PRINT(OPAL_PROC_MY_NAME), (void *) btl, btl->tipc_ifkindex,
#if MCA_BTL_TIPC_STATISTICS
                     btl->tipc_bytes_sent, btl->btl_bytes_recv,
#endif /* MCA_BTL_TIPC_STATISTICS */
                     btl->super.btl_latency, btl->super.btl_bandwidth);
#if OPAL_ENABLE_DEBUG && WANT_PEER_DUMP
    if (NULL != endpoint) {
        MCA_BTL_TIPC_ENDPOINT_DUMP(10, endpoint, false, "TIPC");

    } else if (verbose) {
        opal_list_item_t *item;

        OPAL_THREAD_LOCK(&btl->tipc_endpoints_mutex);
        for (item = opal_list_get_first(&btl->tipc_endpoints);
             item != opal_list_get_end(&btl->tipc_endpoints); item = opal_list_get_next(item)) {
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, (mca_btl_base_endpoint_t *) item, false, "TIPC");
        }
        OPAL_THREAD_UNLOCK(&btl->tipc_endpoints_mutex);
    }
