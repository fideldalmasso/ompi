...
#define MCA_BTL_TIPC_ENDPOINT_CACHE      1
#define MCA_BTL_TIPC_MAGIC_STRING_LENGTH 16
/**
 * State of TIPC endpoint connection.
 */
...
struct mca_btl_base_endpoint_t {
    ...
    struct mca_btl_tipc_module_t *endpoint_btl; /**< BTL instance that created this connection */
    struct mca_btl_tipc_proc_t *endpoint_proc;  /**< proc structure corresponding to endpoint */
    struct mca_btl_tipc_addr_t *endpoint_addr;  /**< address of endpoint */
    ...
    struct mca_btl_tipc_frag_t *endpoint_recv_frag; /**< current recv frag being processed */
...
extern const char mca_btl_tipc_magic_id_string[MCA_BTL_TIPC_MAGIC_STRING_LENGTH];

typedef struct {
    opal_process_name_t guid;
    char magic_id[MCA_BTL_TIPC_MAGIC_STRING_LENGTH];
} mca_btl_tipc_endpoint_hs_msg_t;

void mca_btl_tipc_set_socket_options(int sd);
void mca_btl_tipc_endpoint_close(mca_btl_base_endpoint_t *);
int mca_btl_tipc_endpoint_send(mca_btl_base_endpoint_t *, struct mca_btl_tipc_frag_t *);
void mca_btl_tipc_endpoint_accept(mca_btl_base_endpoint_t *, struct sockaddr *, int);
void mca_btl_tipc_endpoint_shutdown(mca_btl_base_endpoint_t *);

/*
 * Diagnostics: change this to "1" to enable the function
 * mca_btl_tipc_endpoint_dump(), below
 */
#define WANT_PEER_DUMP 0

#if OPAL_ENABLE_DEBUG && WANT_PEER_DUMP
#    define MCA_BTL_TIPC_ENDPOINT_DUMP(LEVEL, ENDPOINT, INFO, MSG) \
        mca_btl_tipc_endpoint_dump((LEVEL), __FILE__, __LINE__, __func__, (ENDPOINT), (INFO), (MSG))
