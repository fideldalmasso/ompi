...
#endif
#ifdef HAVE_NETINET_TIPC_H
#    include <netinet/tipc.h>
#endif
...
const char mca_btl_tipc_magic_id_string[MCA_BTL_TIPC_MAGIC_STRING_LENGTH] = "OPAL-TIPC-BTL";
 ...
 *
 */
static void mca_btl_tipc_endpoint_destruct(mca_btl_tipc_endpoint_t *endpoint)
{
...
OBJ_CLASS_INSTANCE(mca_btl_tipc_endpoint_t, opal_list_item_t, mca_btl_tipc_endpoint_construct,
                   mca_btl_tipc_endpoint_destruct);

static void mca_btl_tipc_endpoint_construct(mca_btl_base_endpoint_t *btl_endpoint);
static void mca_btl_tipc_endpoint_destruct(mca_btl_base_endpoint_t *btl_endpoint);
static int mca_btl_tipc_endpoint_start_connect(mca_btl_base_endpoint_t *);
static void mca_btl_tipc_endpoint_connected(mca_btl_base_endpoint_t *);
...
/**
 * The lack of protection in the mca_btl_tipc_endpoint_dump function is voluntary
 ...
 * to the call.
 */
void mca_btl_tipc_endpoint_dump(int level, const char *fname, int lineno, const char *funcname,
...
#    else
        rcvbuf = -1;
#    endif
#    if defined(TIPC_NODELAY)
        obtlen = sizeof(nodelay);
        if (getsockopt(btl_endpoint->endpoint_sd, IPPROTO_TIPC, TIPC_NODELAY, (char *) &nodelay,
                       &obtlen)
            < 0) {
            BTL_ERROR(
                ("TIPC_NODELAY option: %s (%d)", strerror(opal_socket_errno), opal_socket_errno));
        }
#    else
        nodelay = 0;
#    endif
...
#    endif /* MCA_BTL_TIPC_ENDPOINT_CACHE */
    ...
    if (NULL != btl_endpoint->endpoint_recv_frag)
        used += mca_btl_tipc_frag_dump(btl_endpoint->endpoint_recv_frag, "active recv",
                                      &outmsg[used], DEBUG_LENGTH - used);
    if (used >= DEBUG_LENGTH)
        goto out;

    if (NULL != btl_endpoint->endpoint_send_frag)
        used += mca_btl_tipc_frag_dump(btl_endpoint->endpoint_send_frag,
                                      "active send (inaccurate iov)", &outmsg[used],
                                      DEBUG_LENGTH - used);
    if (used >= DEBUG_LENGTH)
        goto out;
    OPAL_LIST_FOREACH (item, &btl_endpoint->endpoint_frags, mca_btl_tipc_frag_t) {
        used += mca_btl_tipc_frag_dump(item, "pending send", &outmsg[used], DEBUG_LENGTH - used);
        if (used >= DEBUG_LENGTH)
            goto out;
    }
out:
...
/*
 * Initialize events to be used by the endpoint instance for TIPC select/poll callbacks.
 */

static inline void mca_btl_tipc_endpoint_event_init(mca_btl_base_endpoint_t *btl_endpoint)
{
#if MCA_BTL_TIPC_ENDPOINT_CACHE
    assert(NULL == btl_endpoint->endpoint_cache);
    btl_endpoint->endpoint_cache = (char *) malloc(mca_btl_tipc_component.tipc_endpoint_cache);
    btl_endpoint->endpoint_cache_pos = btl_endpoint->endpoint_cache;
#endif /* MCA_BTL_TIPC_ENDPOINT_CACHE */

    opal_event_set(mca_btl_tipc_event_base, &btl_endpoint->endpoint_recv_event,
                   btl_endpoint->endpoint_sd, OPAL_EV_READ | OPAL_EV_PERSIST,
                   mca_btl_tipc_endpoint_recv_handler, btl_endpoint);
    /**
     ...
     */
    opal_event_set(mca_btl_tipc_event_base, &btl_endpoint->endpoint_send_event,
                   btl_endpoint->endpoint_sd, OPAL_EV_WRITE | OPAL_EV_PERSIST,
                   mca_btl_tipc_endpoint_send_handler, btl_endpoint);
}
    ...
    case MCA_BTL_TIPC_CONNECTED:
        if (NULL == btl_endpoint->endpoint_send_frag) {
            if (frag->base.des_flags & MCA_BTL_DES_FLAGS_PRIORITY
                ...
                MCA_BTL_TIPC_ENDPOINT_DUMP(50, btl_endpoint, true,
                                          "complete send fragment [endpoint_send]");
                return 1;
            } else {
                btl_endpoint->endpoint_send_frag = frag;
                MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true,
                                          "event_add(send) [endpoint_send]");
                frag->base.des_flags |= MCA_BTL_DES_SEND_ALWAYS_CALLBACK;
                MCA_BTL_TIPC_ACTIVATE_EVENT(&btl_endpoint->endpoint_send_event, 0);
            }
        } else {
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true,
...
{
    int ret = mca_btl_tipc_send_blocking(btl_endpoint->endpoint_sd, data, size);
    if (ret < 0) {
 ...
 */
static int mca_btl_tipc_endpoint_send_connect_ack(mca_btl_base_endpoint_t *btl_endpoint)
{
    opal_process_name_t guid = opal_proc_local_get()->proc_name;
    OPAL_PROCESS_NAME_HTON(guid);

    mca_btl_tipc_endpoint_hs_msg_t hs_msg;
    opal_string_copy(hs_msg.magic_id, mca_btl_tipc_magic_id_string, sizeof(hs_msg.magic_id));
    hs_msg.guid = guid;

    if (sizeof(hs_msg)
        != mca_btl_tipc_endpoint_send_blocking(btl_endpoint, &hs_msg, sizeof(hs_msg))) {
        opal_show_help("help-mpi-btl-tipc.txt", "client handshake fail", true,
...
static void *mca_btl_tipc_endpoint_complete_accept(int fd, int flags, void *context)
{
    ...
    if (NULL == btl_endpoint->endpoint_addr) {
        ...
        OPAL_THREAD_UNLOCK(&btl_endpoint->endpoint_send_lock);
        OPAL_THREAD_UNLOCK(&btl_endpoint->endpoint_recv_lock);
        if (NULL != btl_endpoint->endpoint_btl->tipc_error_cb) {
            btl_endpoint->endpoint_btl
                ->tipc_error_cb(&btl_endpoint->endpoint_btl->super, MCA_BTL_ERROR_FLAGS_NONFATAL,
    ...
    if ((btl_endpoint->endpoint_sd < 0)
        || (btl_endpoint->endpoint_state != MCA_BTL_TIPC_CONNECTED && cmpval < 0)) {
        mca_btl_tipc_endpoint_close(btl_endpoint);
        btl_endpoint->endpoint_sd = btl_endpoint->endpoint_sd_next;
        btl_endpoint->endpoint_sd_next = -1;
        if (mca_btl_tipc_endpoint_send_connect_ack(btl_endpoint) != OPAL_SUCCESS) {
            MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, true, " [endpoint_accept]");
            btl_endpoint->endpoint_state = MCA_BTL_TIPC_FAILED;
            mca_btl_tipc_endpoint_close(btl_endpoint);
            goto unlock_and_return;
        }
        mca_btl_tipc_endpoint_event_init(btl_endpoint);
        MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "event_add(recv) [endpoint_accept]");
        opal_event_add(&btl_endpoint->endpoint_recv_event, 0);
        if (mca_btl_tipc_event_base == opal_sync_event_base) {
        ...
        MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "accepted");
        goto unlock_and_return;
    }
...
void mca_btl_tipc_endpoint_accept(mca_btl_base_endpoint_t *btl_endpoint, struct sockaddr *addr,
                                 int sd)
{
    struct timeval now = {0, 0};
    ...
    opal_event_evtimer_set(mca_btl_tipc_event_base, &btl_endpoint->endpoint_accept_event,
                           mca_btl_tipc_endpoint_complete_accept, btl_endpoint);
    opal_event_add(&btl_endpoint->endpoint_accept_event, &now);
}
...
{
    MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, false, "[close]");
    if (btl_endpoint->endpoint_sd < 0) {
        return;
    }
    btl_endpoint->endpoint_retries++;
    MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, false, "event_del(recv) [close]");
    ...
    MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, false, "event_del(send) [close]");
    ...
    if (MCA_BTL_TIPC_CONNECTED == btl_endpoint->endpoint_state) {
        mca_btl_tipc_hdr_t fin_msg = {
            .base.tag = 0,
            .type = MCA_BTL_TIPC_HDR_TYPE_FIN,
            .count = 0,
            .size = 0,
        };
        mca_btl_tipc_endpoint_send_blocking(btl_endpoint, &fin_msg, sizeof(fin_msg));
    }
     ...
     */
    if (MCA_BTL_TIPC_FAILED == btl_endpoint->endpoint_state) {
        mca_btl_tipc_frag_t *frag = btl_endpoint->endpoint_send_frag;
        if (NULL == frag) {
            frag = (mca_btl_tipc_frag_t *) opal_list_remove_first(&btl_endpoint->endpoint_frags);
        }
        while (NULL != frag) {
        ...
        if (NULL != btl_endpoint->endpoint_btl->tipc_error_cb) {
            btl_endpoint->endpoint_btl
                ->tipc_error_cb((mca_btl_base_module_t *) btl_endpoint->endpoint_btl, 0,
...
static void mca_btl_tipc_endpoint_connected(mca_btl_base_endpoint_t *btl_endpoint)
{
    /* setup socket options */
    assert(MCA_BTL_TIPC_CONNECTED != btl_endpoint->endpoint_state);
    btl_endpoint->endpoint_state = MCA_BTL_TIPC_CONNECTED;
    btl_endpoint->endpoint_retries = 0;
    MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, true, "READY [endpoint_connected]");

    if (opal_list_get_size(&btl_endpoint->endpoint_frags) > 0) {
        if (NULL == btl_endpoint->endpoint_send_frag) {
            btl_endpoint->endpoint_send_frag = (mca_btl_tipc_frag_t *) opal_list_remove_first(
                &btl_endpoint->endpoint_frags);
        }
        MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "event_add(send) [endpoint_connected]");
 ...
 *  mca_btl_tipc_endpoint_recv_handler().  Don't change them here
 *  without also changing the handling in _recv_handler()!
 */
static int mca_btl_tipc_endpoint_recv_connect_ack(mca_btl_base_endpoint_t *btl_endpoint)
{
    ...
    mca_btl_tipc_endpoint_hs_msg_t hs_msg;
    retval = mca_btl_tipc_recv_blocking(btl_endpoint->endpoint_sd, &hs_msg, sizeof(hs_msg));
    ...
    if (0 != strncmp(hs_msg.magic_id, mca_btl_tipc_magic_id_string, len)) {
        opal_show_help("help-mpi-btl-tipc.txt", "server did not receive magic string", true,
...
// #if defined(TIPC_NODELAY)
//     int optval;
//     optval = !mca_btl_tipc_component.tipc_not_use_nodelay;
//     if (setsockopt(sd, IPPROTO_TIPC, TIPC_NODELAY, (char *) &optval, sizeof(optval)) < 0) {
//         BTL_ERROR(("setsockopt(TIPC_NODELAY) failed: %s (%d)", strerror(opal_socket_errno),
//                    opal_socket_errno));
//     }
// #endif
// #if defined(SO_SNDBUF)
//     if (mca_btl_tipc_component.tipc_sndbuf > 0
//         && setsockopt(sd, SOL_SOCKET, SO_SNDBUF, (char *) &mca_btl_tipc_component.tipc_sndbuf,
//                       sizeof(int))
//                < 0) {
...
//         && setsockopt(sd, SOL_SOCKET, SO_RCVBUF, (char *) &mca_btl_tipc_component.tipc_rcvbuf,
//                       sizeof(int))
//                < 0) {
...
#ifdef FIDEL
    uint16_t af_family = AF_TIPC;
    opal_socklen_t addrlen = sizeof(struct sockaddr_tipc);
#else
...
// #endif
    ...
    mca_btl_tipc_set_socket_options(btl_endpoint->endpoint_sd);
    ...
    mca_btl_tipc_proc_tosocks(btl_endpoint->endpoint_addr, &endpoint_addr);
...
#ifdef FIDEL
// ACA TENGO QUE AGREGAR EL WAIT FOR SERVER DE TIPC
{
	...
	if (0 > connect(sd, (struct sockaddr *)&topsrv, sizeof(topsrv))) {
        BTL_ERROR(("FIDEL Client: failed to connect to topology server.  Failed: %s (%d)",
                 strerror(opal_socket_errno), opal_socket_errno)); 
        return OPAL_ERROR;
	}

	subscr.seq.type = htonl(SERVICE_TYPE);
	subscr.seq.lower = htonl(SERVICE_INSTANCE);
	subscr.seq.upper = htonl(SERVICE_INSTANCE);
	subscr.timeout = htonl(10000);
	subscr.filter = htonl(TIPC_SUB_SERVICE);

	if (send(sd, &subscr, sizeof(subscr), 0) != sizeof(subscr)) {
		BTL_ERROR(("FIDEL Client: failed to send subscription.  Failed: %s (%d)",
                 strerror(opal_socket_errno), opal_socket_errno)); 
        return OPAL_ERROR;
	}
	...
	if (recv(sd, &event, sizeof(event), 0) != sizeof(event)) {
		BTL_ERROR(("FIDEL Client: failed to receive event.  Failed: %s (%d)",
                 strerror(opal_socket_errno), opal_socket_errno)); 
        return OPAL_ERROR;
	}
	if (event.event != htonl(TIPC_PUBLISHED)) {
		printf("FIDEL Client: server {%u,%u} not published within %u [s]\n",
		       SERVICE_TYPE, SERVICE_INSTANCE, 10000/1000);
		BTL_ERROR(("FIDEL Client: server {%u,%u} not published within %u [s].  Failed: %s (%d)",
                SERVICE_TYPE, SERVICE_INSTANCE, 10000/1000,
                 strerror(opal_socket_errno), opal_socket_errno)); 
        return OPAL_ERROR;
	}
	...
	close(sd);
} //FIN DE WAIT FOR SERVER

//ESTA PARTE NO SE SI SE USA
    // if (endpoint_addr.ss_family == AF_TIPC) {
    //     assert(NULL != &btl_endpoint->endpoint_btl->tipc_ifaddr);
    //     if (bind(btl_endpoint->endpoint_sd,
    //              (struct sockaddr *) &btl_endpoint->endpoint_btl->tipc_ifaddr,
    //              sizeof(struct sockaddr_tipc ))
    //         < 0) {
    //         BTL_ERROR(
    //             ("FIDEL bind on local address (%s:XDDD) failed: %s (%d)",
    //              opal_net_get_hostname((struct sockaddr *) &btl_endpoint->endpoint_btl->tipc_ifaddr),
    //              strerror(opal_socket_errno), opal_socket_errno));
        ...
        if (bind(btl_endpoint->endpoint_sd,
            ...
            BTL_ERROR(
                ("bind on local address (%s:%d) failed: %s (%d)",
                 opal_net_get_hostname((struct sockaddr *) &btl_endpoint->endpoint_btl->tipc_ifaddr),
                 htons(((struct sockaddr_in *) &btl_endpoint->endpoint_btl->tipc_ifaddr)->sin_port),
                 strerror(opal_socket_errno), opal_socket_errno));
...
//                  opal_net_get_hostname((struct sockaddr *) &btl_endpoint->endpoint_btl->tipc_ifaddr),
//                  htons(
//                      ((struct sockaddr_in6 *) &btl_endpoint->endpoint_btl->tipc_ifaddr)->sin6_port),
//                  strerror(opal_socket_errno), opal_socket_errno));
...
// #endif
    opal_output_verbose(10, opal_btl_base_framework.framework_output,
                        "btl: tipc: attempting to connect() to %s address %s on port %d",
                        OPAL_NAME_PRINT(btl_endpoint->endpoint_proc->proc_opal->proc_name),
    ...
    if (0 == connect(btl_endpoint->endpoint_sd, (struct sockaddr *) &endpoint_addr, addrlen)) {
...
#ifdef FIDEL
        opal_output_verbose(10, opal_btl_base_framework.framework_output,
                            "FIDEL btl:tipc: connect() to %s:XDDDD completed",
                            opal_net_get_hostname((struct sockaddr *) &endpoint_addr));
#else
        opal_output_verbose(10, opal_btl_base_framework.framework_output,
                            "btl:tipc: connect() to %s:%d completed",
                            opal_net_get_hostname((struct sockaddr *) &endpoint_addr),
                            ntohs(((struct sockaddr_in *) &endpoint_addr)->sin_port));
#endif
        /* send our globally unique process identifier to the endpoint */
        if ((rc = mca_btl_tipc_endpoint_send_connect_ack(btl_endpoint)) == OPAL_SUCCESS) {
            btl_endpoint->endpoint_state = MCA_BTL_TIPC_CONNECT_ACK;
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "event_add(recv) [start_connect]");
            opal_event_add(&btl_endpoint->endpoint_recv_event, 0);
            if (mca_btl_tipc_event_base == opal_sync_event_base) {
                /* If no progress thread then raise the awarness of the default progress engine */
                opal_progress_event_users_increment();
         ...
         */
        MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, true, "dropped connection [start_connect]");
    } else {
        /* non-blocking so wait for completion */
        if (opal_socket_errno == EINPROGRESS || opal_socket_errno == EWOULDBLOCK) {
            btl_endpoint->endpoint_state = MCA_BTL_TIPC_CONNECTING;
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "event_add(send) [start_connect]");
            MCA_BTL_TIPC_ACTIVATE_EVENT(&btl_endpoint->endpoint_send_event, 0);
            opal_output_verbose(30, opal_btl_base_framework.framework_output,
                                "btl:tipc: would block, so allowing background progress");
            return OPAL_SUCCESS;
        }
    }
 ...
 */
static int mca_btl_tipc_endpoint_complete_connect(mca_btl_base_endpoint_t *btl_endpoint)
{
    int so_error = 0;
    ...
    if (getsockopt(btl_endpoint->endpoint_sd, SOL_SOCKET, SO_ERROR, (char *) &so_error, &so_length)
        ...
        opal_show_help("help-mpi-btl-tipc.txt", "socket flag fail", true, opal_process_info.nodename,
                       ...
                       opal_socket_errno);
#ifdef FIDEL
        BTL_ERROR(("FIDEL getsockopt() to %s:XDDD failed: %s (%d)",
                   opal_net_get_hostname((struct sockaddr *) &endpoint_addr),strerror(opal_socket_errno),
                   opal_socket_errno));
#else
    ...
    if (so_error != 0) {
        if (mca_btl_base_warn_peer_error || mca_btl_base_verbose > 0) {
            char *msg;
#ifdef FIDEL
            opal_asprintf(&msg, "FIDEL connect() to %s:XDDD failed",
                          opal_net_get_hostname((struct sockaddr *) &endpoint_addr));
#else
            opal_asprintf(&msg, "connect() to %s:%d failed",
                          ...
                          ntohs(((struct sockaddr_in *) &endpoint_addr)->sin_port));
#endif
            opal_show_help("help-mpi-btl-tipc.txt", "client connect fail", true,
    ...
    opal_output_verbose(
        ...
        "btl:tipc: connect() to %s:XDDD completed (complete_connect), sending connect ACK",
        opal_net_get_hostname((struct sockaddr *) &endpoint_addr));
#else
    opal_output_verbose(
        10, opal_btl_base_framework.framework_output,
        "btl:tipc: connect() to %s:%d completed (complete_connect), sending connect ACK",
        opal_net_get_hostname((struct sockaddr *) &endpoint_addr),
        ntohs(((struct sockaddr_in *) &endpoint_addr)->sin_port));
#endif
    ...
    if (mca_btl_tipc_endpoint_send_connect_ack(btl_endpoint) == OPAL_SUCCESS) {
        ...
        MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, false, "event_add(recv) [complete_connect]");
        return OPAL_SUCCESS;
    }
    MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, false, " [complete_connect]");
    btl_endpoint->endpoint_state = MCA_BTL_TIPC_FAILED;
    mca_btl_tipc_endpoint_close(btl_endpoint);
    return OPAL_ERROR;
}
...
static void mca_btl_tipc_endpoint_recv_handler(int sd, short flags, void *user)
{
    ...
    switch (btl_endpoint->endpoint_state) {
    case MCA_BTL_TIPC_CONNECT_ACK: {
        int rc = mca_btl_tipc_endpoint_recv_connect_ack(btl_endpoint);
        if (OPAL_SUCCESS == rc) {
            /* we are now connected. Start sending the data */
            OPAL_THREAD_LOCK(&btl_endpoint->endpoint_send_lock);
            mca_btl_tipc_endpoint_connected(btl_endpoint);
            OPAL_THREAD_UNLOCK(&btl_endpoint->endpoint_send_lock);
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, true, "connected");
            ...
            /* Otherwise, it probably *was* an OMPI peer process on
               ...
               happened.  */
            mca_btl_tipc_module_t *m = btl_endpoint->endpoint_btl;

            /* Fail up to the PML */
            if (NULL != m->tipc_error_cb) {
                m->tipc_error_cb(
                    (mca_btl_base_module_t *) m, MCA_BTL_ERROR_FLAGS_FATAL,
                    btl_endpoint->endpoint_proc->proc_opal,
                    "TIPC ACK is neither SUCCESS nor ERR (something bad has probably happened)");
            }
        }
        OPAL_THREAD_UNLOCK(&btl_endpoint->endpoint_recv_lock);
        return;
    ...
    case MCA_BTL_TIPC_CONNECTED: {
        ...
        if (NULL == frag) {
            if (mca_btl_tipc_module.super.btl_max_send_size
                ...
                MCA_BTL_TIPC_FRAG_ALLOC_MAX(frag);
            } else {
                MCA_BTL_TIPC_FRAG_ALLOC_EAGER(frag);
            }
...
static void mca_btl_tipc_endpoint_send_handler(int sd, short flags, void *user)
{
    ...
    case MCA_BTL_TIPC_CONNECTED:
        ...
        while (NULL != btl_endpoint->endpoint_send_frag) {
            ...
            assert(btl_endpoint->endpoint_state == MCA_BTL_TIPC_CONNECTED);
        ...
        if (NULL == btl_endpoint->endpoint_send_frag) {
            MCA_BTL_TIPC_ENDPOINT_DUMP(10, btl_endpoint, false,
                                      "event_del(send) [endpoint_send_handler]");
            opal_event_del(&btl_endpoint->endpoint_send_event);
        }
        break;
    case MCA_BTL_TIPC_FAILED:
        MCA_BTL_TIPC_ENDPOINT_DUMP(1, btl_endpoint, true,
