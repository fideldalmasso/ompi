...
OBJ_CLASS_INSTANCE(mca_btl_tipc_proc_t, opal_list_item_t, mca_btl_tipc_proc_construct,
                   mca_btl_tipc_proc_destruct);

void mca_btl_tipc_proc_construct(mca_btl_tipc_proc_t *tipc_proc)
{
    tipc_proc->proc_opal = NULL;
    ...
    OBJ_CONSTRUCT(&tipc_proc->btl_index_to_endpoint, opal_hash_table_t);
    opal_hash_table_init(&tipc_proc->btl_index_to_endpoint, mca_btl_tipc_component.tipc_num_btls);
}
...
void mca_btl_tipc_proc_destruct(mca_btl_tipc_proc_t *tipc_proc)
{
    ...
    if (NULL != tipc_proc->proc_addrs) {
        free(tipc_proc->proc_addrs);
    }
    OBJ_DESTRUCT(&tipc_proc->btl_index_to_endpoint);
    OBJ_DESTRUCT(&tipc_proc->proc_lock);
}

static inline int mca_btl_tipc_proc_is_proc_left(opal_process_name_t a, opal_process_name_t b)
{
    if (a.jobid != b.jobid) {
...
#define MCA_BTL_TIPC_PROC_LOCAL_VERTEX(index)  (index)
#define MCA_BTL_TIPC_PROC_REMOTE_VERTEX(index) (index + mca_btl_tipc_component.tipc_num_btls)
 ...
 *
 */
static int mca_btl_tipc_proc_create_interface_graph(mca_btl_tipc_proc_t *btl_proc,
                                                   mca_btl_tipc_modex_addr_t *remote_addrs,
                                                   int local_proc_is_left,
                                                   opal_bp_graph_t **graph_out)
{
    opal_bp_graph_t *graph = NULL;
    opal_reachable_t *results = NULL;
    opal_list_t *local_list = &mca_btl_tipc_component.local_ifs;
    ...
    for (i = 0; i < btl_proc->proc_addr_count; i++) {
        ...
        if (MCA_BTL_TIPC_AF_INET == remote_addrs[i].addr_family) {
        ...
        } else if (MCA_BTL_TIPC_AF_INET6 == remote_addrs[i].addr_family) {
// #if OPAL_ENABLE_IPV6
...
// #endif
    ...
    for (x = 0; x < results->num_local; x++) {
        rc = opal_bp_graph_add_vertex(graph, &mca_btl_tipc_component.tipc_btls[x]->btl_index,
                                      &v_index);
        if (OPAL_SUCCESS != rc) {
            goto err_graph;
        }
    ...
    for (x = 0; x < results->num_local; x++) {
        for (y = 0; y < results->num_remote; y++) {
            ...
            if (local_proc_is_left) {
                u = MCA_BTL_TIPC_PROC_LOCAL_VERTEX(x);
                v = MCA_BTL_TIPC_PROC_REMOTE_VERTEX(y);
            } else {
                u = MCA_BTL_TIPC_PROC_REMOTE_VERTEX(y);
                v = MCA_BTL_TIPC_PROC_LOCAL_VERTEX(x);
            }
 ...
 * a pointer to a mca_btl_tipc_addr_t struct.
 */
static int mca_btl_tipc_proc_store_matched_interfaces(mca_btl_tipc_proc_t *btl_proc,
                                                     ...
                                                     int num_matched, int *matched_edges)
{
    ...
    struct mca_btl_tipc_addr_t *remote_addr;
...
static int mca_btl_tipc_proc_handle_modex_addresses(mca_btl_tipc_proc_t *btl_proc,
                                                   mca_btl_tipc_modex_addr_t *remote_addrs,
                                                   int local_proc_is_left)
{
    opal_bp_graph_t *graph = NULL;
    ...
    rc = mca_btl_tipc_proc_create_interface_graph(btl_proc, remote_addrs, local_proc_is_left,
                                                 &graph);
    if (rc) {
        goto cleanup;
    }
    ...
    rc = mca_btl_tipc_proc_store_matched_interfaces(btl_proc, local_proc_is_left, graph, num_matched,
                                                   matched_edges);
    if (rc) {
        goto cleanup;
    }
 ...
 * between a opal_proc_t and a mca_btl_tipc_proc_t instance. We cache
 * additional data (specifically the list of mca_btl_tipc_endpoint_t instances,
 * and published addresses) associated w/ a given destination on this
 * datastructure.
 */

mca_btl_tipc_proc_t *mca_btl_tipc_proc_create(opal_proc_t *proc)
{
    mca_btl_tipc_proc_t *btl_proc;
    int rc, local_proc_is_left;
    mca_btl_tipc_modex_addr_t *remote_addrs = NULL;
    size_t size;

    OPAL_THREAD_LOCK(&mca_btl_tipc_component.tipc_lock);
    rc = opal_proc_table_get_value(&mca_btl_tipc_component.tipc_procs, proc->proc_name,
                                   (void **) &btl_proc);
    if (OPAL_SUCCESS == rc) {
        OPAL_THREAD_UNLOCK(&mca_btl_tipc_component.tipc_lock);
        return btl_proc;
    ...
    OPAL_MODEX_RECV(rc, &mca_btl_tipc_component.super.btl_version, &proc->proc_name,
    ...
    if (0 != (size % sizeof(mca_btl_tipc_modex_addr_t))) {
        BTL_ERROR(("opal_modex_recv: invalid size %lu: btl-size: %lu\n", (unsigned long) size,
                   (unsigned long) sizeof(mca_btl_tipc_modex_addr_t)));
        rc = OPAL_ERROR;
        goto cleanup;
    }

    btl_proc->proc_addr_count = size / sizeof(mca_btl_tipc_modex_addr_t);
    btl_proc->proc_addrs = malloc(btl_proc->proc_addr_count * sizeof(mca_btl_tipc_addr_t));
     ...
     */
    local_proc_is_left = mca_btl_tipc_proc_is_proc_left(proc->proc_name,
                                                       opal_proc_local_get()->proc_name);
    rc = mca_btl_tipc_proc_handle_modex_addresses(btl_proc, remote_addrs, local_proc_is_left);
...
cleanup:
    if (OPAL_SUCCESS == rc) {
        ...
        opal_proc_table_set_value(&mca_btl_tipc_component.tipc_procs, proc->proc_name, btl_proc);
    } else {
        if (btl_proc) {
 ...
 * it an address.
 */
int mca_btl_tipc_proc_insert(mca_btl_tipc_proc_t *btl_proc, mca_btl_base_endpoint_t *btl_endpoint)
{
    mca_btl_tipc_module_t *tipc_btl = btl_endpoint->endpoint_btl;
    mca_btl_tipc_addr_t *remote_addr;
    int rc = OPAL_SUCCESS;

    rc = opal_hash_table_get_value_uint32(&btl_proc->btl_index_to_endpoint, tipc_btl->btl_index,
                                          (void **) &remote_addr);
    if (OPAL_SUCCESS != rc) {
        if (9 < opal_output_get_verbosity(opal_btl_base_framework.framework_output)) {
            ...
            opal_output(0, "btl:tipc: host %s, process %s UNREACHABLE", proc_hostname,
 ...
 * process identifier.
 */
mca_btl_tipc_proc_t *mca_btl_tipc_proc_lookup(const opal_process_name_t *name)
{
    mca_btl_tipc_proc_t *proc = NULL;

    OPAL_THREAD_LOCK(&mca_btl_tipc_component.tipc_lock);
    opal_proc_table_get_value(&mca_btl_tipc_component.tipc_procs, *name, (void **) &proc);
    OPAL_THREAD_UNLOCK(&mca_btl_tipc_component.tipc_lock);
    if (OPAL_UNLIKELY(NULL == proc)) {
        mca_btl_base_endpoint_t *endpoint;
        opal_proc_t *opal_proc;

        BTL_VERBOSE(("adding tipc proc for peer {%s}", OPAL_NAME_PRINT(*name)));
        ...
        for (uint32_t i = 0; i < mca_btl_tipc_component.tipc_num_btls; ++i) {
            endpoint = NULL;
            (void) mca_btl_tipc_add_procs(&mca_btl_tipc_component.tipc_btls[i]->super, 1, &opal_proc,
                                         &endpoint, NULL);
 ...
 * of the request.
 */
void mca_btl_tipc_proc_accept(mca_btl_tipc_proc_t *btl_proc, struct sockaddr *addr, int sd)
{
    OPAL_THREAD_LOCK(&btl_proc->proc_lock);
    int found_match = 0;
    ...
    for (size_t i = 0; i < btl_proc->proc_endpoint_count; i++) {
         ...
         * end we prepare for the real decision function mca_btl_tipc_endpoint_accept. */
...
#ifdef FIDEL
        case AF_TIPC:{
            if (memcmp(&btl_endpoint->endpoint_addr->addr_union.addr_tipc,
                       &(((struct sockaddr_tipc *) addr)->addr.name.name), sizeof(struct tipc_name))) {
                char tmp[2][16];
                opal_output_verbose(20, opal_btl_base_framework.framework_output,
                                    "btl: tipc: Match incoming connection from %s XD with locally "
                                    "known IP XD failed (iface %d/%d)!\n",
                                    OPAL_NAME_PRINT(btl_proc->proc_opal->proc_name),
                                    (int) i, (int) btl_proc->proc_endpoint_count);
                continue;
            } else if (btl_endpoint->endpoint_state != MCA_BTL_TIPC_CLOSED) {
                found_match = 1;
        ...
        case AF_INET:
            if (memcmp(&btl_endpoint->endpoint_addr->addr_union.addr_inet,
                ...
                char tmp[2][16];
                opal_output_verbose(20, opal_btl_base_framework.framework_output,
                                    "btl: tipc: Match incoming connection from %s %s with locally "
...
// #endif
        ...
        (void) mca_btl_tipc_endpoint_accept(btl_endpoint, addr, sd);
        OPAL_THREAD_UNLOCK(&btl_proc->proc_lock);
        return;
    }
    /* In this case the connection was inbound to an address exported, but was not in a CLOSED
     * state. mca_btl_tipc_endpoint_accept() has logic to deal with the race condition that has
     * likely caused this scenario, so call it here.*/
    if (found_match) {
        (void) mca_btl_tipc_endpoint_accept(match_btl_endpoint, addr, sd);
        OPAL_THREAD_UNLOCK(&btl_proc->proc_lock);
        return;
    }
    ...
    CLOSE_THE_SOCKET(sd);
    {
        ...
        opal_show_help("help-mpi-btl-tipc.txt", "dropped inbound connection", true,
...
/*
 * convert internal data structure (mca_btl_tipc_addr_t) to sockaddr_storage
 *
 */
bool mca_btl_tipc_proc_tosocks(mca_btl_tipc_addr_t *proc_addr, struct sockaddr_storage *output)
{
    memset(output, 0, sizeof(*output));
    switch (proc_addr->addr_family) {
#ifdef FIDEL
    case AF_TIPC: {
        struct sockaddr_tipc *inaddr = (struct sockaddr_tipc *) output;
        output->ss_family = AF_TIPC;
        inaddr->addrtype = proc_addr->addrtype;
        memcpy(&((struct sockaddr_tipc *) output)->addr.nameseq, &proc_addr->nameseq, sizeof(struct tipc_name_seq));
        memcpy(&((struct sockaddr_tipc *) output)->addr.name.name, &proc_addr->addr_union.addr_tipc, sizeof(struct tipc_name));
        break;
    }
#endif
    case AF_INET:
...
// #endif
    default:
        opal_output(0, "mca_btl_tipc_proc: unknown af_family received: %d\n",
